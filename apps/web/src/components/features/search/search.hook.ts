...
import type { TSearchConfig, Service, SearchFilters } from '../types/search.types'
...
export const useSearch = ({ query, category, location }: TSearchConfig) => {
  ...
  const filteredServices = useMemo(() => {
    ...
    if (filters.availability !== 'any') {
      ...
      filtered = filtered.filter(service => service.nextSlot && service.nextSlot.includes('Hoje'))
    }
