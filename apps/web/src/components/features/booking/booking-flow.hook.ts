...
export type TUseBookingFlowReturn = {
  ...
  handleProfessionalChange: (professional: TProfessional | null, isAny: boolean) => void
...
export const useBookingFlow = ({ onBack }: TUseBookingFlowConfig): TUseBookingFlowReturn => {
  ...
  const [selectedProfessional, setSelectedProfessional] = useState<TProfessional | null>(null)
  ...
  const totalDuration = useMemo(
    () => selectedServices.reduce((total, service) => total + (service.duration * service.quantity), 0),
    [selectedServices]
  )
  ...
  const handleProfessionalChange = useCallback((professional: TProfessional | null, isAny: boolean) => {
    setSelectedProfessional(professional)
    setIsAnyProfessional(isAny)
    setValidationError(null)
  }, [])

  const handleDateTimeChange = useCallback((date: Date | undefined, time: string | null) => {
  ...
  }, [currentStep, selectedServices, selectedProfessional, isAnyProfessional, selectedDate, selectedTime])
