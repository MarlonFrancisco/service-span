...
import { ChevronLeft, ChevronRight, Calendar, Clock, User, X, CheckCircle, RotateCcw, Settings } from "lucide-react";
...
export function AgendaModule({ activeStore }: AgendaModuleProps) {
  ...
  const getAppointmentForSlot = (professional: string, day: number, time: string) => {
  ...
  const getSlotColor = (appointment: Appointment | null) => {
    if (!appointment) return 'bg-[#20b2aa]/10 hover:bg-[#20b2aa]/20 border-[#20b2aa]/30';
  ...
  const handleAppointmentAction = (action: 'complete' | 'cancel' | 'reschedule') => {
  ...
  const handleMouseDown = useCallback((professional: string, dayIndex: number, time: string) => {
  ...
  const handleMouseEnter = useCallback((professional: string, dayIndex: number, time: string) => {
  ...
  const handleSlotClick = (professional: string, dayIndex: number, time: string) => {
  ...
  const isSlotBlocked = (professional: string, dayIndex: number, time: string) => {
    const slotId = `${professional}-${dayIndex}-${time}`;
    return blockedSlots.has(slotId);
  };

  const isSlotSelected = (professional: string, dayIndex: number, time: string) => {
    const slotId = `${professional}-${dayIndex}-${time}`;
      ...
      <div className="min-w-[800px]">
        ...
        <div className="grid grid-cols-8 gap-1 mb-4">
          ...
          {days.map((day, index) => {
            ...
            return (
              <div key={day} className={`p-3 text-center ${!isWorking ? 'opacity-50' : ''}`}>
                <div className="text-sm text-[#1a2b4c]">{day}</div>
                <div className="text-xs text-gray-500">
                  {new Date(Date.now() + (currentWeek * 7 + index) * 24 * 60 * 60 * 1000)
          ...
          <div key={professional.id} className="mb-6">
            <div className="grid grid-cols-8 gap-1">
              ...
              <div className="p-3 bg-gray-50 rounded-lg flex items-center">
                <div>
                  <div className="text-sm text-[#1a2b4c]">{professional.name}</div>
              ...
              {days.map((day, dayIndex) => {
                ...
                return (
                  <div key={dayIndex} className={`space-y-1 ${!isWorking ? 'opacity-50' : ''}`}>
                    {timeSlots.slice(0, 8).map((time) => {
                      const appointment = getAppointmentForSlot(professional.name, dayIndex, time);
                      const isBlocked = isSlotBlocked(professional.name, dayIndex, time);
                      const isSelected = isSlotSelected(professional.name, dayIndex, time);
                      
                      let slotClass = '';
                      if (!isWorking) {
                        slotClass = 'bg-gray-200 border-gray-300 cursor-not-allowed';
                      } else if (appointment) {
                        slotClass = getSlotColor(appointment);
                      } else if (isBlocked) {
                        slotClass = 'bg-gray-300 border-gray-400 text-gray-600';
                      } else if (isSelected) {
                        slotClass = 'bg-[#1a2b4c]/30 border-[#1a2b4c] ring-2 ring-[#1a2b4c]/50';
                      } else {
                        slotClass = 'bg-[#20b2aa]/10 hover:bg-[#20b2aa]/20 border-[#20b2aa]/30';
                      }
                      ...
                      return (
                        <div
                          ...
                          className={`
                            ...
                            ${slotClass}
                          `}
                          onMouseDown={() => isWorking && handleMouseDown(professional.name, dayIndex, time)}
                          onMouseEnter={() => isWorking && handleMouseEnter(professional.name, dayIndex, time)}
                          onClick={() => isWorking && handleSlotClick(professional.name, dayIndex, time)}
                        >
                          ...
                          {!isWorking && (
                            <div className="text-xs text-center mt-1">Fechado</div>
                          )}
                        </div>
                      );
                    })}
    ...
    <div className="space-y-6">
      <div className="text-center">
        <h3 className="text-lg text-[#1a2b4c] mb-2">
          {new Date(Date.now() + (currentWeek * 7) * 24 * 60 * 60 * 1000)
        ...
        <Card key={professional.id}>
          ...
          <CardContent>
            <div className="grid grid-cols-4 gap-2">
              {timeSlots.map((time) => {
                const appointment = getAppointmentForSlot(professional.name, 0, time);
        ...
        <div className="grid grid-cols-7 gap-2">
          ...
          {monthDays.map(day => {
            const dayAppointments = mockAppointments.filter(apt => 
              professionalsToShow.some(prof => prof.name === apt.professional)
            );
            
            return (
              <div key={day} className="min-h-[80px] p-2 border rounded-lg bg-white hover:bg-gray-50">
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 text-sm">Gerencie os agendamentos da unidade selecionada</p>
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          <Tabs value={viewType} onValueChange={(value) => setViewType(value as 'week' | 'day' | 'month')}>
            <TabsList>
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-[#1a2b4c] min-w-[100px] text-center">
              {currentWeek === 0 ? 'Esta Semana' : 
               currentWeek > 0 ? `+${currentWeek} semanas` : `${currentWeek} semanas`}
            </span>
            <Button 
              variant="outline" 
              size="sm"
          ...
          <Card>
            <CardHeader>
              ...
              <p className="text-sm text-gray-600">
                Clique para bloquear/desbloquear horários ou arraste para selecionar múltiplos
              </p>
            </CardHeader>
            <CardContent>
              {renderCalendarView()}
      ...
      <Dialog open={!!selectedAppointment} onOpenChange={() => setSelectedAppointment(null)}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle className="text-[#1a2b4c]">Detalhes do Agendamento</DialogTitle>
          </DialogHeader>
            ...
            <div className="space-y-4">
              <div className="space-y-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm">Cliente: {selectedAppointment.clientName}</span>
                </div>
                ...
                <div className="bg-gray-50 p-3 rounded-lg">
                  <p className="text-sm text-[#1a2b4c]">{selectedAppointment.service}</p>
                ...
                <div className="flex items-center gap-2">
                  <Badge variant={
                    ...
                    selectedAppointment.status === 'completed' ? 'secondary' : 'destructive'
                  }>
      ...
      <Dialog open={isSettingsOpen} onOpenChange={setIsSettingsOpen}>
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-6">
            <div>
              <h4 className="text-sm text-[#1a2b4c] mb-4">Dias de Funcionamento</h4>
              <div className="space-y-3">
                {days.map((day, index) => (
