  ...
  const generateWeekData = (weekOffset: number): DaySchedule[] => {
    ...
    return days.map((dayName, dayIndex) => {
      ...
      const slots: TimeSlot[] = times.map((time, timeIndex) => {
        ...
        if (dayIndex === 1 && timeIndex === 4) {
          // Tuesday 11:00
          status = 'booked';
          clientName = 'Ana Silva';
          service = 'Corte + Escova';
        } else if (dayIndex === 1 && timeIndex === 5) {
          // Tuesday 11:30
          status = 'booked';
          clientName = 'Ana Silva';
          service = 'Corte + Escova';
          ...
          // Thursday 13:00
          status = 'blocked';
        } else if (dayIndex === 0 && timeIndex === 6) {
          // Monday 12:00
          status = 'booked';
  ...
  return (
    ...
    >
      ...
      <div className="mt-4 text-sm text-gray-600 bg-blue-50 p-3 rounded-lg">
        <strong>Como usar:</strong> Clique em um horário livre para
        bloquear/desbloquear, ou clique e arraste para bloquear múltiplos
        horários de uma vez.
      </div>
    </div>
  );
